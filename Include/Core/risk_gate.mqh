...
private:
    int GetLocalHour();
    double GetATR();
    double CalculateMaxRiskPips(double riskAmount, double atr);
...
//| Is Spread OK                                                      |
//+------------------------------------------------------------------+
bool CRiskGate::IsSpreadOK() {
    double spread = SymbolInfoInteger(m_symbol, SYMBOL_SPREAD) * SymbolInfoDouble(m_symbol, SYMBOL_POINT);
    double spreadPts = spread / SymbolInfoDouble(m_symbol, SYMBOL_POINT);
    
    // Check fixed max
    if(spreadPts > m_spreadMaxPts) {
        return false;
    }
    
    // Check ATR-based
    double atr = GetATR();
    double maxSpreadATR = atr * m_spreadATRpct;
...
//| Is Rollover Time                                                  |
//+------------------------------------------------------------------+
bool CRiskGate::IsRolloverTime() {
    datetime now = TimeCurrent();
    MqlDateTime dt;
    TimeToStruct(now, dt);
    
    // Rollover at 00:00 ±5 minutes (GMT+7)
    if(dt.hour == 0 && dt.min >= 0 && dt.min <= 5) {
        return true;
    }
...
//| Get Max Risk Pips                                                 |
//+------------------------------------------------------------------+
double CRiskGate::GetMaxRiskPips() {
    double balance = AccountInfoDouble(ACCOUNT_BALANCE);
    double riskAmount = balance * (m_riskPct / 100.0);
    double atr = GetATR();
    return CalculateMaxRiskPips(riskAmount, atr);
}
...
double CRiskGate::GetMaxLotSize() {
    double balance = AccountInfoDouble(ACCOUNT_BALANCE);
    double riskAmount = balance * (m_riskPct / 100.0);
    double maxRiskPips = GetMaxRiskPips();
    return CalculateMaxLotSize(riskAmount, maxRiskPips);
}
